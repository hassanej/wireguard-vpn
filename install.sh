...
ensure_ios_client() {
    ...
    [ -n "$client_id" ] ||
        fail "The permanent iOS client could not be created."
}

create_first_identity_backup() {
    local state_path="${REPO_ROOT}/${STATE_RELATIVE_PATH}"

    echo "Creating the permanent WireGuard identity archive locally..."
    ...
    if ! tar -C "$DATA_DIR" -czf "$state_path" .; then
        docker compose \
            -f "${INSTALL_DIR}/compose.yaml" \
            start wg-easy || true

        fail "Could not create the WireGuard state archive."
    fi
    ...
    [ -s "$state_path" ] ||
        fail "The WireGuard state archive is empty."

    tar -tzf "$state_path" >/dev/null 2>&1 ||
        fail "The WireGuard state archive failed validation."

    echo
    echo "Permanent WireGuard identity created."
    echo "Archive: ${state_path}"
    echo
    echo "Copy this archive to your Mac repository:"
    echo "  ${STATE_RELATIVE_PATH}"
    echo
    echo "Then commit and push it manually."
    echo "Do not destroy this VPS until the archive is safely committed."
}

if [ "${EUID}" -ne 0 ]; then
    fail "Run this installer as root (sudo --preserve-env=DUCKDNS_TOKEN bash install.sh)."
fi

require_secret DUCKDNS_TOKEN

[ -r /etc/os-release ] ||
    fail "This installer requires Ubuntu."

# shellcheck disable=SC1091
source /etc/os-release

[ "${ID:-}" = "ubuntu" ] ||
    fail "This installer supports Ubuntu only."

command -v systemctl >/dev/null 2>&1 ||
    fail "A systemd-based Ubuntu server is required."
...
[ "$DUCKDNS_RESULT" = "OK" ] ||
    fail "DuckDNS rejected the update."
...
mkdir -p \
    "$DATA_DIR" \
    "${INSTALL_DIR}/caddy_data" \
    "${INSTALL_DIR}/caddy_config"

chmod 700 "$DATA_DIR"

if [ -s "${REPO_ROOT}/${STATE_RELATIVE_PATH}" ]; then
    tar -tzf "${REPO_ROOT}/${STATE_RELATIVE_PATH}" >/dev/null 2>&1 ||
        fail "The committed WireGuard state archive is invalid."
    ...
    tar -C "$DATA_DIR" \
        -xzf "${REPO_ROOT}/${STATE_RELATIVE_PATH}"
fi
...
HASH_OUTPUT=$(
    docker run --rm "$WG_EASY_IMAGE" wgpw "$WG_PASSWORD"
)
...
[ -n "$PASSWORD_HASH" ] ||
    fail "Could not generate the dashboard password hash."

PASSWORD_HASH=${PASSWORD_HASH//\$/\$\$}

cat >"${INSTALL_DIR}/compose.yaml" <<COMPOSE_EOF
services:
  wg-easy:
    image: ${WG_EASY_IMAGE}
...
if [ "$STATE_PRESENT" = false ]; then
    echo "The permanent identity archive was created locally."
    echo "Copy and commit it before destroying this VPS:"
    echo "  ${REPO_ROOT}/${STATE_RELATIVE_PATH}"
    echo
else
    echo "The committed WireGuard identity was restored."
fi
